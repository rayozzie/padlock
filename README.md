...
Except for this comment, Padlock was 100% vibe-coded by Ray Ozzie over the course of several days in April 2025 as a means of trying to understand the limits of the technique. For better or worse, this involved many hundreds of instructions given to multiple models over many hours, with corrections and rewrites.  I found that I could only accomplish the task through a "mixture of models" (MoM) technique, moving the project from model to model when one got stuck, and having them check each other's work.  The models used were primarily ChatGPT o3-mini-high, Devin (which I believe is currently on Sonnet 3.7), and Claude 3.7 Sonnet directly, and Gemini Advanced 2.5 Pro.  Ultimately I succeeded, but for a SINGLE line of code that was generated by o3-mini-high and which none of the models was able to successfully recognize and fix: the golang buf = append(pad[n:m], input...) was overwriting the 'pad' slice because the pad had ample capacity. (That line of code was eventually rewritten by the model in a later refactor.)
