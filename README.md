...
Except for this comment, Padlock was vibe-coded over the course of several days in early 2025 as a learning exercise and a means of trying to understand the capabilities and constraints. For better or worse, this involved many hundreds of written instructions issued to multiple models over many hours, with corrections and rewrites.  As compared with what's now possible with modern CLI-based tooling, the process was needlessly painful because of the inability for the default UI's to natively integrate with my file system.  

Although I started with a fairly concise description of what I wanted, I found that I could only get the design and implementation correct the task through a "mixture of models" technique, moving the project from model to model when one got stuck, and having them check and re-do each other's work.  A simple observation is that for each and all of them, once the context started to get too long and they got stuck, it was best to start fresh and train it in my goals all over again, with the current code as its new starting point.
...
While at first I began with Devin (Sonnet 3.7), I soon moved to ChatGPT o3-mini-high and kept going back and forth between 3.7 Sonnet directly in Claude's UI, through Claude Code once discovered, and a bit of Gemini Advanced 2.5 Pro. 
